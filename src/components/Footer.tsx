...
//             <li><Link to="/clientes" className="hover:text-yellow-400">Clientes</Link></li>
//           </ul>
//         </div>
  ...
  return (
    <footer className="bg-gray-900 text-white px-6 py-10 mt-12">
      <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-6 text-sm">
        ...
        {/* Logo e información */}
        <div>
          <img src="/logo/fabriconstruc_dark.png" alt="Fabriconstruc Logo" className="h-12 mb-3" />
          <p>
            Soluciones en estructuras metálicas, obras civiles e ingeniería en el sur del Perú.
          </p>
        </div>

        {/* Enlaces reutilizados */}
        <div>
          <h4 className="font-semibold mb-2">Enlaces</h4>
          <ul className="space-y-1">
            {links.map(({ to, label }) => (
              <li key={to}>
                <Link to={to} className="hover:text-yellow-400">{label}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          <h4 className="font-semibold mb-2">Contacto</h4>
          <p className="mb-1">Correo: fabriconstruc2010@gmail.com</p>
          <p className="mb-1">Teléfonos: 944 210 427 / 983 811 193</p>
          <p className="mb-1">Dirección: Tingo Grande, Jacobo Hunter – Arequipa</p>
        </div>
      </div>
